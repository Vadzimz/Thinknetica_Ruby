...
class Railway
  ...
  def choose_option
    opts = ["make_a_station", 
      ...
      "get_info_on_trains_placed_in_station",
      "get_list_of_carriages_of_a_train",
      "occupy_a_seat_in_a_passenger_train",
      "fill_some_volume_in_a_cargo_train",
      "exit_menu"]

    i = choose(opts, "option", true, &DEFAULT_OUTPUT)
  ...
  def remove_a_middle_station_from_a_route
    ...
    if route.stations.count > 2
      i = choose(route.stations[1..-2], "middle station", true, &STATION_OUTPUT)
  ...
  def make_a_train
    begin
      ...
      id = gets.chomp
      i = choose(["passenger", "cargo"], "type of train", true, &DEFAULT_OUTPUT)
  ...
  def get_list_of_carriages_of_a_train
    return puts "No trains_to_choose" if trains.empty?
    train = choose(trains, "train", &TRAIN_OUTPUT)
    case train.type
    when :passenger
  ...
  def get_info_on_trains_placed_in_station
    return puts "No stations_to_choose" if stations.empty?
  ...
  def occupy_a_seat_in_a_passenger_train
    ...
    available_carriages = train.carriages.select{|car| car.vacant_seats > 0}
  ...
  def fill_some_volume_in_a_cargo_train
    ...
    return puts "No carriages to choose" if train.carriages.empty?
    available_carriages = train.carriages.select{|car| car.free_volume > 0}
  ...
  ROUTE_OUTPUT = proc {|v, i| puts "#{i} : departure station: #{v.stations.first.name}  -  destination station: #{v.stations.last.name}"}
  STATION_OUTPUT = proc {|v, i| puts "#{i} : #{v.name}, #{v.trains.size} trains"}
  TRAIN_OUTPUT = proc {|v, i| puts "#{i}: train id: #{v.id}, type: #{v.type}, quantity of carriages: #{v.carriages.size}"}
  PASSENGER_CAR_OUTPUT = 
    proc {|v, i| puts "carriage_number: #{i}, type: #{v.type}, vacant_seats: #{v.vacant_seats}, busy_seats: #{v.busy_seats}"}
  CARGO_CAR_OUTPUT = 
    proc {|v, i| puts "carriage_number: #{i}, type: #{v.type}, free_volume: #{v.free_volume}, filled_volume: #{v.filled_volume}"}
  ...
  def seed
    ...
    def st(name)
      self.stations.find {|st| st.name == name}
    end

    def tr(id)
      self.trains.find {|t| t.id == id}
    end
    ...
    tr("001-01").set_route(routes[0])
    tr("002-01").set_route(routes[1])
    tr("003-01").set_route(routes[2])
    tr("001-02").set_route(routes[0])
    tr("002-02").set_route(routes[1])
    tr("003-02").set_route(routes[2])
    ...
    return puts "Seed data is created"
